...
These variables can only have one value when applied to various objects. These variables can be shared by all class members since they are not 
...
Recovering from Errors is not possible.                                       | Recover from exceptions by either using a try-catch block or 
                                                                                throwing exceptions back to the caller.
...
Errors can occur at compile time as well as run time.                         | All exceptions occur at runtime but checked exceptions are known to the compiler 
